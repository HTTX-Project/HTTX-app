<template>
    <view class="container">
        ...
        <view class="carousel-section">
            ...
            <swiper class="carousel" autoplay="true" interval="6000" duration="1000" circular @change="swiperChange">
        ...
        <view class="cate-section">
            <view class="cate-item" @click="goNavTo('notice')">
                <image src="/static/temp/homegg.png"></image>
                <text>平台咨询</text>
            </view>
            <view class="cate-item" @click="goNavTo('sharingInvitations')">
                <image src="/static/temp/ic1.png"></image>
                <text>邀请新人</text>
            </view>
            <view class="cate-item" @click="goNavTo('myBill')">
                <image src="/static/temp/c1.png"></image>
                <text>我的账单</text>
            </view>
            <view class="cate-item" @click="goNavTo('myIncome')">
                <image src="/static/temp/homerr.png"></image>
                <text>我的收益</text>
            </view>
        ...
        <!--                <swiper-item class="g-swiper-item" v-for="(item, index) in goodsList" :key="index"-->
        <!--                             v-if="index % 2 === 0" @click="navToDetailPage(item)">-->
...
<!--            <text class="yticon icon-you"></text>-->
        ...
        <view class="hot-floor">
            ...
            <scroll-view class="floor-list" scroll-x>
                <view class="scoll-wrapper">
                    <view v-for="(item, index) in topGoodsList" :key="index" class="floor-item"
                          @click="navToDetailPage(item)">
        ...
        <view class="f-header m-t">
            <image src="/static/temp/h1.png"></image>
            <view class="tit-box">
                <text class="tit">全部商品</text>
                <text class="tit2">All merchandise</text>
            </view>
<!--            <text class="yticon icon-you"></text>-->
        </view>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      //获取全部商品
      http.get(`/goodImpl/goodListByType?USER_ID=${this.user.USER_ID}&TYPE_ONE_ID=&TYPE_TWO_ID=&keywords=`, []).then((res) => {
        self.goodsList = res.data.goodList
      }).catch(error => {
        console.log('首页商品', error)
      });
      //获取推荐商品
      http.get(`/goodImpl/topGoodList?USER_ID=${this.user.USER_ID}`, []).then((res) => {
        self.topGoodsList = res.data.goodList
      }).catch(error => {
        console.log('推荐商品', error)
      });
    },
    getBanner(){
      const self = this
      //banner
      http.get(`/goodImpl/bannerList`, []).then((res) => {
        self.carouselList = res.data.bannerList
      }).catch(error => {
        console.log('推荐商品', error)
      });
    },
    ...
    navToDetailPage(item) {
      ...
      // })
      this.$mRouter.push({
        route: this.$mRouterConfig.product,
        query: {
          id: item.USER_ID
        }
      })
