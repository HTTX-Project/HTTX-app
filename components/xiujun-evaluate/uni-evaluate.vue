	...
	<view class="evaluateBox">
		...
		<view class="lists" v-if="listData.length > 0">
			<block v-for="(item, index_) in listData" :key="index_">
				<view class="item">
					<view class="icon"><image :src="item.WX_IMG" mode="widthFix" style="width:100%" /></view>
					<view class="info">
						<view class="name-time">
							<text class="name">{{ item.VIP_NAME }}</text>
						...
						<view class="stars">
							...
							<view class="stars-selected" :style="{ width: (item.PL_STAR / 5) * 100 + '%' }">
								<view class="box">
						...
						<view class="evaluate-content">
							<text>{{ item.PL_CONTENT || '用户暂未评价' }}</text>
				...
				<view class='zghf' v-if="item.ADMIN_BACK != '' && item.ADMIN_BACK">
				 	<view class="t">掌柜回复:</view>
					 <text class="f">{{item.ADMIN_BACK}}</text>
				</view>
			</block>
		</view>
